    ...
    tarefas.forEach(tarefa => {
        ...
        taskConfigBtns.forEach(btn => {
            btn.addEventListener('click', function () {
                ...
                editTaskBtns.classList.toggle('show');
            });
        });
...
function resetData() {
    ...
    const screen = document.getElementById("wipe-screen");
    const fill = document.querySelector(".wipe-fill");

    // inicia animação de tela
    screen.classList.add("active");

    setTimeout(() => {
        fill.style.animation = "wipe-progress 1s linear forwards";
    }, 50);

    // delay para apagar dados (ex.: 500 ms)
    // você pode ajustar para o valor que quiser
    setTimeout(() => {
        localStorage.clear();
    }, 600); // só apaga meio segundo depois da animação começar

    // recarrega a página depois que tudo já acabou
    setTimeout(() => {
        location.reload();
    }, 1400); 
}
